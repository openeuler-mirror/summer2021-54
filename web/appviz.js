...
function createGraphViz() {
    ...
    forwardInfo[struct_input].forEach(function (node,index) {
        if (!nodes_state.has(node)) {
            ...
            port = getPort(struct_input, node)
            graph += `struct:pf${port} -> struct_f${index}:p; \n`
        }else{
    ...
    backwardInfo[struct_input].forEach(function (node,index) {
        if (!nodes_state.has(node)) {
            ...
            port = getPort(node, struct_input)
            graph += `struct_b${index}:pf${port} -> struct:p; \n`
        }else if (nodes_state.get(node) != "center"){
            port = getPort(node, struct_input)
            graph += `struct_${nodes_state.get(node)}:pf${port} -> struct:p; \n`
        }
    });
...
function genDot(struct){
    ...
    dotTable = `[label=< 
        ...
        <TR><TD BALIGN="CENTER" COLSPAN="2" PORT="p"> ${struct} </TD></TR> \n`
